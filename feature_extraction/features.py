...
import sys
reload(sys);
sys.path.insert(0, "/projets/iris/PROJETS/PRINCESS/code/libs/readability")
...
def getDocContent(trecid):
    '''function en cours'''
    princess_dir = '/projets/iris/PROJETS/PRINCESS'
...
def pagerank_loaded(id):
    conn = sqlite3.connect("/projets/iris/PROJETS/PRINCESS/page_rank/pagerank.db")
    cursor = conn.cursor()
...
def pagerank(id):
    file = "/projets/iris/PROJETS/PRINCESS/page_rank/pagerank.scoreOrder.bz2"
...
def spam(id):
    conn = sqlite3.connect("/projets/iris/PROJETS/PRINCESS/spam_score/spam.db")
    cursor = conn.cursor()
